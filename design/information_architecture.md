...
PARENT MODE (bottom nav: Today · Activities · Sound · Goals · Learn)
 ...
 │   └─ Session player (cue cards) → Session log                                          REQ-M2-01..03
 ├─ Activities       by goal → Activity detail (steps, pause prompt, tier, example ▶)     REQ-M1-01..04
 │   └─ Routines     routine list → routine (visual schedule, song ▶, record my voice)    REQ-M3-01..03
 ...
 ├─ Coach            chat (from Today and Learn)                                          REQ-AI-02..05
...
CHILD MODE (no nav bar; Stop bar on every screen; exit only through the parental gate)
 ...
 ├─ Calm corner (child playlist, sway visual, child volume)                               REQ-M4-01..03
