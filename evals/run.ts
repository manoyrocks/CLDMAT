...
// coach_test_v2.json: second blind set, never used for tuning; reported as the final test result.
...
const test = evalExternal('coach_test_v2.json');
...
writeFileSync(here('./results.json'), JSON.stringify({ runAt: new Date().toISOString(), mode: 'offline-extractive', metrics, heldOutDev: dev, heldOutTest: test, targets: Object.fromEntries(targets), failures, libraryBlocked: falseBlocks }, null, 2));
...
for (const [name, r] of [['Held-out DEVELOPMENT set (coach_heldout.json; used to fix DEF-010)', dev], ['Held-out FINAL TEST set (coach_test_v2.json; never tuned on; report only)', test]] as const) {
  if (!r) continue;
