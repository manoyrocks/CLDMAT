...
const CRISIS: RegExp[] = [
  ...
  /\b(drank|drunk|drink|ate|eaten|swallowed)\b[^.?!]{0,25}\b(bleach|detergent|dishwashing|cleaner|chemical|poison|medicine|pills|liquid)\b/,
  /\bpoison/, /\bhit (his|her|their) head\b/, /\b(head injury|concussion)\b/, /\bvomit\w*\b[^.?!]{0,40}\b(sleepy|drowsy|confused)\b/,
  /\b(car|bus|lorry|truck|motorbike|bike) (hit|knocked|ran over) (him|her|them)\b/, /\b(bleeding|lots of blood)\b/,
...
const MEDICATION: RegExp[] = [
  ...
  /\b(a|one|half|\d+) tablets?\b/, /\b(drops|pills?)\b[^.?!]{0,20}\b(calm|sleep|before|give|take)\b/, /\b(give|take)\b[^.?!]{0,20}\b(drops|pills?)\b/,
];
const CURE: RegExp[] = [
  ...
  /\b(not|no longer|isn'?t|won'?t be)\b[^.?!]{0,12}\b(autistic|on the spectrum)\b/, /\bback to normal\b/,
...
const STIM_ACT = /\b(stim\w*|flap\w*|rock(s|ing)?\b(?! music)|spin(s|ning)?|twirl\w*|hum(s|ming)?|flick\w*|pacing|jump(s|ing)? up and down|repetitive|vocal (stim|sounds?|noises?)|(same|repeat\w*) (sound|noise|word)s?|pag-?ikot|pag-?flap|e{3,}|quiet hands)\b/;
const STIM_STOP = /\b(stop|stops|stopping|reduce|get rid|prevent|extinguish|decrease|quit|replace|train (him|her|them) out|out of (his|her|their) stims?|hold (his|her|their) hands?|quiet hands|reward chart|sticker|mapapatigil|patigilin|tigil|less|blend in|make (him|her|them) (stop|quit))\b/;
  ...
  const refusal: RefusalCategory | null =
    DIAGNOSIS.some((r) => r.test(q)) ? 'diagnosis'
      : MEDICATION.some((r) => r.test(q)) ? 'medication'
        : CURE.some((r) => r.test(q)) ? 'cure'
          : null;
  return {
    crisis: CRISIS.some((r) => r.test(q)),
    injection: INJECTION.some((r) => r.test(q)),
    refusal,
    stimming: STIM_ACT.test(q) && STIM_STOP.test(q),
